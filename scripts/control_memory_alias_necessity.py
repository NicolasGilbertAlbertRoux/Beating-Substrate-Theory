...
# -*- coding: utf-8 -*-
"""
BST canonical supplementary script: control_memory_alias_necessity

Category: BST memory anti-cheat control
...
#     coherent memory-bounded arbitration  >> observable-only
#     coherent memory-bounded arbitration  >> speed-only-no-memory
#     coherent memory-bounded arbitration  >> destroyed / wrong / shuffled memory
...
#
# Outputs:
#   results/summaries/observable_alias_memory_necessity/
# ============================================================
...
OUTDIR = "../results/summaries/observable_alias_memory_necessity"
SEED = 41341
...
class Config:
    ...
    arbitration_strength: float = 0.55
...
def make_episode(rng, cfg: Config, alias_leak: float):
    ...
    arbitration = unit(
        cfg.arbitration_strength * memory
        + 0.22 * identities
        + 0.12 * observable
        + cfg.noise * rng.normal(size=(cfg.n_states, cfg.dim))
    )
...
def reconstruct(observable_vec, memory_vec, arbitration_vec, mode, cfg: Config):
    ...
    if mode == "speed_only_no_memory":
        ...
        speed_gate = 1.0 / (1.0 + np.exp(-6.0 * (np.linalg.norm(arbitration_vec) - 0.95)))
        return unit(cfg.observable_strength * observable_vec + 0.12 * speed_gate * arbitration_vec)

    if mode == "latent_memory_only":
        return memory_vec

    if mode == "arbitration_only":
        return arbitration_vec

    if mode == "memory_bounded_arbitration":
        gate = max(0.0, cosine(memory_vec, arbitration_vec))
        return unit(
            cfg.observable_strength * observable_vec
            + cfg.memory_strength * memory_vec
            + gate * cfg.arbitration_strength * arbitration_vec
        )

    if mode == "overdriven_arbitration":
        return unit(0.05 * observable_vec + 0.20 * memory_vec + 1.35 * arbitration_vec)
...
def run():
    ...
    modes = [
        ...
        "memory_bounded_arbitration",
        "latent_memory_only",
        "arbitration_only",
        "overdriven_arbitration",
                ...
                for target_idx in range(cfg.n_states):
                    ...
                    for memory_condition in memory_conditions:
                        if memory_condition == "coherent_memory":
                            ...
                            arbitration = episodes[swap_idx]["arbitration"]
                        ...
                        arbitration_vec = unit(
                            memory_cap * arbitration[target_idx]
                            + (1.0 - memory_cap) * observable_vec
                        )

                        for mode in modes:
                            candidate = reconstruct(observable_vec, memory_vec, arbitration_vec, mode, cfg)
                            ...
                            common = {
                                ...
                                "arbitration_similarity": cosine(arbitration_vec, identities[target_idx]),
    ...
    full = val("memory_bounded_arbitration", "coherent_memory")
    ...
    arbitration_only = val("arbitration_only", "coherent_memory")
    destroyed = val("memory_bounded_arbitration", "destroyed_memory")
    wrong = val("memory_bounded_arbitration", "wrong_coherent_memory")
    shuffled = val("memory_bounded_arbitration", "shuffled_memory")
    adversarial = val("memory_bounded_arbitration", "adversarial_memory")
    swap = val("memory_bounded_arbitration", "memory_swap")
    ...
    print(f"arbitration_only_recovery               {arbitration_only:.4f}")
